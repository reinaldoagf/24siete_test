...
export default function Items() {
    return (
      <ul className="p-1.5 flex flex-wrap">
        {Elements.map(({ key, artist, image, price, title, timeLeft }) => (
          <li className="w-full lg:w-1/2 xl:w-1/3  p-1.5" key={key}>
            <a
              className="block bg-zinc-200 rounded-md w-full overflow-hidden pb-4 shadow-lg"
              href="#items"
            >
              <div
              ...
              <div className="flex mt-2">
                ...
                <div className="p-3 w-1/2">
                  <button className="bg-gradient-to-tr from-fuchsia-600 to-violet-600  w-full rounded-md font-semibold h-12 p-px">
                    <div className="bg-zinc-300 w-full h-full rounded-md grid place-items-center">
                      View artwork
                    </div>
                  </button>
                </div>
